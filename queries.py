...
def Q_1(cursor, conn, execution_time):
    ...
    query = """ 
        SELECT
            p.player_name,
            ROUND(AVG(s.statsbomb_xg),8) AS average_xg
        FROM
            matches m
        JOIN
            events e ON m.match_id = e.match_id
        JOIN
            shot s ON e.index = s.index AND e.match_id = s.match_id
        JOIN
...
def Q_3(cursor, conn, execution_time):
    ...
    query = """ 
        SELECT
            p.player_name,
            COUNT(s.*) AS total_first_time_shots
        FROM
            matches m
        JOIN
            events e ON m.match_id = e.match_id
...
def Q_4(cursor, conn, execution_time):
    ...
    query = """
        SELECT
            t.team_name,
            COUNT(p.*) AS total_passes
        FROM
            matches m
        JOIN
            events e ON m.match_id = e.match_id
        JOIN
            pass p ON e.index = p.index AND e.match_id = p.match_id
        JOIN
...
def Q_5(cursor, conn, execution_time):
    ...
    query = """
        SELECT
            pl.player_name,
            COUNT(p.recipient_id) AS pass_count
        FROM
            Pass p
        JOIN
            Player pl ON p.recipient_id = pl.player_id
        JOIN
...
def Q_6(cursor, conn, execution_time):
    ...
    query = """ 
        SELECT
            t.team_name,
            COUNT(s.match_id) AS shot_count
        FROM
            Shot s
        JOIN
            Matches m ON s.match_id = m.match_id
        JOIN
            Team t ON t.team_id = m.home_team_id OR t.team_id = m.away_team_id
        JOIN
            Competition c ON m.competition_id = c.competition_id AND m.season_id = c.season_id
        WHERE
            c.competition_name = 'Premier League'
            AND c.season_name = '2003/2004'
        GROUP BY
...
def Q_7(cursor, conn, execution_time):
    ...
    query = """ 
        ...
        JOIN
            events e ON m.match_id = e.match_id
        JOIN
            pass ps ON e.index = ps.index AND e.match_id = ps.match_id
        JOIN
            player p ON e.player_id = p.player_id
        JOIN
            competition c ON m.competition_id = c.competition_id
        WHERE
            c.competition_name = 'La Liga'
            AND c.season_name = '2020/2021'
            AND ps.technique_name = 'Through Ball'
        GROUP BY
            p.player_name
        ORDER BY
...
def Q_8(cursor, conn, execution_time):
    ...
    query = """ 
        ...
        JOIN
            pass ps ON e.index = ps.index AND e.match_id = ps.match_id
        JOIN
            team t ON e.possession_team_id = t.team_id
        JOIN
            competition c ON m.competition_id = c.competition_id
        WHERE
            c.competition_name = 'La Liga'
...
def Q_9(cursor, conn, execution_time):
    ...
    query = """ 
        ...
        JOIN
            events e ON m.match_id = e.match_id
        JOIN
            dribble d ON e.index = d.index AND e.match_id = d.match_id
        JOIN
            player p ON e.player_id = p.player_id
        JOIN
            competition c ON m.competition_id = c.competition_id
...
def Q_10(cursor, conn, execution_time):
    ...
    query = """ 
        SELECT
            p.player_name,
            COUNT(*) AS num_dribbled_past
        FROM
            matches m
        JOIN
            events e ON m.match_id = e.match_id
        JOIN
            dribbled_past dp ON e.index = dp.index AND e.match_id = dp.match_id
        JOIN
            player p ON e.player_id = p.player_id
        JOIN
            competition c ON m.competition_id = c.competition_id
